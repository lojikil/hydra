...
(define (null? n) (eq? n '()))
(define (pair? n) (type? n "PAIR"))
(define (vector? n) (type? n "VECTOR"))
(define (dict? n) (type? n "DICT"))
(define (symbol? n) (type? n "ATOM"))
(define (key? n) (type? n "KEY"))
(define (number? n) (type? n "NUMBER"))
(define (string? n) (type? n "STRING"))
(define (bool? n) (type? n "BOOL"))
(define (goal? n) (type? n "GOAL"))
(define (not x)
        (cond
            (eq? x #s) #u
            (eq? x #f) #t
        ...
        (if (= (car dump) 0)
            ...
            (let ((top-dump (cadr dump))
                ...
                (typhon@vm
                    ...
                    (+ (vector-ref top-dump (- offset 3)) 1)
                    (cons (car stack) (vector-ref top-dump (- offset 4)))
                    (vector-ref top-dump (- offset 5))
                                ...
                                (typhon@lambda? call-proc)
                                    ...
                                    (if (> (car dump) (length (cadr dump)))
                                        ...
                                        (let ((env-and-stack (build-environment (vector-ref (cadr call-proc) 1) stack (vector-ref (cadr call-proc) 2) locals))
                                            ...
                                            (vector-set! v-dump (+ offset 4) code)
                                            (typhon@vm
